    ...
    if (showDateRangePicker) {
        DateRangePickerModal(
            onDateRangeSelected = { selectedDates ->
                val startDateMillis = selectedDates.first
                val endDateMillis = selectedDates.second

                if (startDateMillis == null || endDateMillis == null) {
                    Toast.makeText(context, "Please select a valid date range", Toast.LENGTH_SHORT).show()
                } else {
                    arrivalDate = LocalDate.ofEpochDay(startDateMillis / (1000 * 60 * 60 * 24))
                    departureDate = LocalDate.ofEpochDay(endDateMillis / (1000 * 60 * 60 * 24))
                }
